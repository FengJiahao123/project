...
<script>window.onload=function(){window.print();window.close()}<\/script></body></html>`)
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setChatMessages((prev) => [
        ...prev,
        { role: 'ai', text: resp.message, changes: resp.changes_summary },
      ])
      setPendingScript(resp.modified_script)
    } catch (e) {
      setChatMessages((prev) => [
        ...prev,
        { role: 'ai', text: `❌ ${e instanceof Error ? e.message : '修改失败，请重试'}` },
      ])
    } finally {
  ...
  return (
    <div>
      ...
      {/* Main Content */}
      <div className={`flex gap-0 ${chatOpen ? '' : ''}`}>
        {/* Script Panel */}
        <div
          className={`${chatOpen ? 'w-1/2 border-r border-gray-200' : 'w-full'}`}
        >
          <div
          ...
          >
            <div className="p-6 max-w-[600px] mx-auto" style={{ fontSize: '14px', lineHeight: '1.9' }}>
              ...
              <div className="text-center mb-8">
                <h2 className={`text-xl font-bold ${nameColor}`}>
                ...
                return (
                  <div key={scene.scene_number} className="mb-8">
                    ...
                    {scene.elements.map((el, i) => {
                      if (el.type === 'action') {
                        return <p key={i} className={`mb-3 ${actionColor} text-justify`}>{el.content}</p>
                      }
                      if (el.type === 'transition') {
                        return <p key={i} className={`mb-4 text-right text-sm ${transitionColor}`}>{el.content}</p>
                      }
                      if (el.type === 'dialogue') {
                        const name = charMap.get(el.speaker) || el.speaker
                        return (
                          <div key={i} className="mb-4">
                            <p className={`text-center font-bold uppercase tracking-[0.2em] text-sm ${nameColor} mb-1`}>{name}</p>
                            {el.emotion && <p className={`text-center text-xs mb-1 ${emotionColor}`}>（{el.emotion}）</p>}
                            <div className="max-w-[360px] mx-auto">
                              {el.lines.map((line, j) => (
                                <p key={j} className={`text-center ${dialogueColor} mb-0.5`}>{line}</p>
                              ))}
                            </div>
                            {el.notes && <p className={`text-center text-xs mt-1 ${emotionColor}`}>（{el.notes}）</p>}
                          </div>
                        )
                      }
                      return null
        ...
        {chatOpen && (
          <div className="w-1/2 flex flex-col bg-gray-50 rounded-lg border border-gray-200 ml-2 max-h-[600px]">
            ...
            <div className="flex-1 overflow-auto p-3 space-y-3">
              ...
              {chatLoading && (
                <div className="text-sm text-gray-400 p-2 mr-8">
                  🤖 AI 正在修改...
                </div>
              )}
              ...
              {pendingScript && (
                <div className="flex gap-2 mt-2">
                  ...
                  <button
                    ...
                    className="flex-1 px-3 py-2 bg-gray-200 text-gray-700 text-xs font-medium rounded-lg hover:bg-gray-300 transition-colors"
                  >
                    ↩ 撤销
                  </button>
                </div>
            ...
            <div className="p-3 border-t border-gray-200 bg-white rounded-b-lg">
              <div className="flex gap-2">
                <textarea
                  className="flex-1 p-2 border border-gray-300 rounded-lg text-sm resize-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                  rows={2}
